...
fn initial_mapping_is_close_to_identity() {
    ...
    let mut apm = AdaptiveProbabilityMap::new(
        full_count, stretched_fract_index_bits, &squash_lut);
    ...
    for index in 1..full_count {
        ...
        assert!(diff_sq < 0.000_000_000_5, "{}", diff_sq);
        assert!(diff_st < 0.000_000_5, "{}", diff_st);
        ...
        apm.update_predictions(index - 1, Bit::One, 1, false);
        previous = refined_sq;
    }
}
    ...
    for &(orig_model_freq, orig_real_freq, max_overhead) in [
        ...
        for &(model_freq, real_freq, flipped) in [
            ...
            for round in 0..10 {
                ...
                let mut apm = AdaptiveProbabilityMap::new(
                    1, stretched_fract_index_bits, &squash_lut);
                let warmup = 100_000;
...
fn mapping_is_symmetric() {
    ...
    for &(model_freq, real_freq) in [
        ...
        for _ in 0..1000 {
            ...
            assert!(asymmetry_fract <= 1, "{}", asymmetry_fract);
            let a_input_bit: Bit = (prng.next_real2() >= real_freq).into();
            let b_input_bit: Bit = !a_input_bit;
            a_apm.update_predictions(0, a_input_bit, 10, false);
            b_apm.update_predictions(0, b_input_bit, 10, false);
        }
    }
}
